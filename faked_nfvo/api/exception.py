...
class UnsupportedCinderAPIVersion(NovaException):
    msg_fmt = ('Nova does not support Cinder API version %(version)s')
...
class CinderAPIVersionNotAvailable(NovaException):
    ...
    """
    msg_fmt = ('Cinder API version %(version)s is not available.')
...
class ExtendVolumeNotSupported(Invalid):
    msg_fmt = ("Volume size extension is not supported by the hypervisor.")
...
class ApiVersionsIntersect(Invalid):
    msg_fmt = ("Version of %(name)s %(min_ver)s %(max_ver)s intersects "
...
class InvalidNUMANodesNumber(Invalid):
    msg_fmt = ("The property 'numa_nodes' cannot be '%(nodes)s'. "
...
class ServiceNotUnique(Invalid):
    msg_fmt = ("More than one possible service found.")
...
class VolumeAttachmentNotFound(NotFound):
    msg_fmt = ("Volume attachment %(attachment_id)s could not be found.")
...
class PortUpdateFailed(Invalid):
    msg_fmt = ("Port update failed for port %(port_id)s: %(reason)s")
...
class FixedIpInvalidOnHost(Invalid):
    msg_fmt = ("The fixed IP associated with port %(port_id)s is not "
...
class ConfGroupForServiceTypeNotFound(ServiceNotFound):
    msg_fmt = ("No conf group name could be found for service type "
                "%(stype)s.")
...
class QuotaClassExists(NovaException):
    msg_fmt = ("Quota class %(class_name)s exists for resource %(resource)s")
...
class QuotaUsageRefreshNotAllowed(Invalid):
    msg_fmt = ("Quota usage refresh of resource %(resource)s for project "
...
class RPCPinnedToOldVersion(NovaException):
    msg_fmt = ("RPC is pinned to old version")
...
class OnsetFilePathLimitExceeded(OnsetFileLimitExceeded):
    msg_fmt = ("Personality file path exceeds maximum %(allowed)s")


class OnsetFileContentLimitExceeded(OnsetFileLimitExceeded):
    msg_fmt = ("Personality file content exceeds maximum %(allowed)s")
...
class VolumeRebaseFailed(NovaException):
    msg_fmt = ("Volume rebase failed: %(reason)s")
...
class SecurityProxyNegotiationFailed(NovaException):
    msg_fmt = ("Failed to negotiate security type with server: %(reason)s")
...
class ImageNUMATopologyAsymmetric(Invalid):
    msg_fmt = ("Instance CPUs and/or memory cannot be evenly distributed "
...
class CPUPinningInvalid(Invalid):
    msg_fmt = ("CPU set to pin %(requested)s must be a subset of "
                "free CPU set %(free)s")


class CPUUnpinningInvalid(Invalid):
    msg_fmt = ("CPU set to unpin %(requested)s must be a subset of "
...
class InstanceDiagnosticsNotSupported(Invalid):
    msg_fmt = ("Instance diagnostics are not supported by compute node.")
...
class InvalidResourceClass(Invalid):
    msg_fmt = ("Resource class '%(resource_class)s' invalid.")
...
class InvalidResourceAmount(Invalid):
    msg_fmt = ("Resource amounts must be integers. Received '%(amount)s'.")
...
class NotSupportedWithOption(Invalid):
    msg_fmt = ("%(operation)s is not supported in conjunction with the "
...
class NeutronAdminCredentialConfigurationInvalid(Invalid):
    msg_fmt = ("Networking client is experiencing an unauthorized exception.")


class PlacementNotConfigured(NovaException):
    msg_fmt = ("This compute is not configured to talk to the placement "
                "service. Configure the [placement] section of nova.conf "
                "and restart the service.")


class InvalidEmulatorThreadsPolicy(Invalid):
    msg_fmt = ("CPU emulator threads option requested is invalid, "
                "given: '%(requested)s', available: '%(available)s'.")


class BadRequirementEmulatorThreadsPolicy(Invalid):
    msg_fmt = ("An isolated CPU emulator threads option requires a dedicated "
                "CPU policy option.")


class PowerVMAPIFailed(NovaException):
    msg_fmt = ("PowerVM API failed to complete for instance=%(inst_name)s.  "
                "%(reason)s")


class TraitNotFound(NotFound):
    msg_fmt = ("No such trait %(name)s.")
