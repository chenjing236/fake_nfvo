...
from cloudtest.tests.ceph_api.lib import http
...
class RestClient(object):
    """Unified RestClient class for Ceph mgmt system REST API test.
    ...
    def __init__(self, endpoint, disable_ssl_certificate_validation=False,
                 ca_certs=None, trace_requests='', http_timeout=None):
    ...
    def expected_success(cls, expected_code, read_code):
        ...
        """
        cls.LOG.info("Start verifying HTTP status code")
        ...
        if isinstance(expected_code, list):
            for code in expected_code:
                if not code in HTTP_SUCCESS + HTTP_REDIRECTION:
                    raise exceptions.InvalidHttpSuccessCode(assert_msg)
        else:
            if not expected_code in HTTP_SUCCESS + HTTP_REDIRECTION:
                raise exceptions.InvalidHttpSuccessCode(assert_msg)
    ...
    def _log_request_start(self, method, req_url):
        caller_name = utils.find_test_caller()
...
"""
    ...
    def response_checker(self, method, resp, resp_body):
        ...
        if (resp.status == 205 and
                    0 != len(set(resp.keys()) - set(('status',)) -
                                 self.response_header_lc -
                                 self.general_header_lc)):
    ...
    def _construct_url(self, url, headers, body):
        return (self.endpoint + url, headers, body)
    ...
    def _request(self, method, url, headers=None, body=None, chunked=False):
        ...
        req_url, req_headers, req_body = self._construct_url(url, headers, body)
    ...
    def _error_checker(self, resp, resp_body):
        ...
        if resp.status == 410:
            if parse_resp:
                resp_body = self._parse_resp(resp_body)
            raise exceptions.Gone(resp_body)
    ...
    def validate_response(cls, schema, resp, body):
        ...
        if resp.status_code in HTTP_SUCCESS + HTTP_REDIRECTION:
            cls.expected_success(schema['status_code'], resp.status_code)

            cls.LOG.info("Start validating response body")
